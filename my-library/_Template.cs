...
public static partial class MyLib {
  ...
  [MI(R256)] public static List<T> PopRange<T>(this List<T> list, Range rng) { var (off, len) = rng.GetOffsetAndLength(list.Count); var res = list.GetRange(off, len); list.RemoveRange(off, len); return res; }
  [MI(R256)] public static (int[] res, T[] conv) Compress<T>(ReadOnlySpan<T> span) where T : IComparable<T>, IEquatable<T> { var conv = span.ToArray(); Array.Sort(conv); int n = span.Length, l = 0, r = 0; while (r < n) { T v = conv[l++] = conv[r]; while (r < n && conv[r].Equals(v)) r++; } Array.Resize(ref conv, l); var res = new int[n]; for (int i = 0; i < res.Length; i++) res[i] = conv.LowerBound(span[i]); return (res, conv); }
  ...
  [MI(R256)] public static string RepeatStr(this char c, int count) => string.Create(count, c, static (dest, src) => dest.Fill(src));
  [MI(R256)] public static string RepeatStr(this string s, int count) => string.Create(s.Length * count, s, static (dest, src) => { for (int i = 0; i < dest.Length / src.Length; i++) { src.CopyTo(dest.Slice(i * src.Length, src.Length)); } });
