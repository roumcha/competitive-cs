...
public static partial class MyLib {
  ...
  [MI(R256)] public static T[,] RotateRight90<T>(this T[,] src) { int len0 = src.GetLength(0), len1 = src.GetLength(1); var res = new T[len1, len0]; for (int i = 0; i < len0; i++) { for (int j = 0; j < len1; j++) res[j, len0 - 1 - i] = src[i, j]; } return res; }
