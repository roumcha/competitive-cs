public static partial class Mylib {
  ...
  public static void RotateLeft90<T>(this T[,] src) {
    ...
    if (n != m) throw new ArgumentException($"width ({n}) must be the same as height ({m}), or use ToRotatedLeft90()");
    for (int i = 0; i < n / 2; i++) {
  ...
  public static void RotateRight90<T>(this T[,] src) {
    int n = src.GetLength(0), m = src.GetLength(1);
    if (n != m) throw new ArgumentException($"width ({n}) must be the same as height ({m}), or use ToRotatedRight90()");
    for (int i = 0; i < n / 2; i++) {
      for (int j = i; j < n - i - 1; j++) {
        T t = src[i, j];
        src[i, j] = src[n - 1 - j, i];
